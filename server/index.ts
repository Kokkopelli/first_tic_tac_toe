...
// Load configuration from JSON file (følger retningslinjene)
let config: any = {};
const configFilePath = process.env.CONFIG_FILE_PATH || "./config/secrets.json";
...
(async () => {
  ...
  // For production deployment, use port 80 (standard HTTP port)
  // this serves both the API and the client.
  const port = parseInt(process.env.PORT || config.app_port || '80', 10);
  server.listen({
    port,
    host: "0.0.0.0",
    reusePort: true,
